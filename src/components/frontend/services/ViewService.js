...
function ViewService() {
    ...
    useEffect(() => {
        ...
        document.title = "List of Service";
    ...
    const filtereservice = ViewService.filter((ser) =>
    ...
    if (loading) {
        ...
        var ServiceStatus = '';
        var badgeClass = ''
        dispaly_Servicedata = filtereservice.map((item) => {
            if (item.active == '0') {
            ...
            return (
                <tr key={item.id}>
                    <td>{item.id}</td>
                    <td className="table-plus"><img src={`http://localhost:8000/${item.image}`} width={200} height={0} alt={item.name} /></td>
                    <td>{item.name}</td>
                    <td>{item.location}</td>
                    <td>{item.adresse}</td>
                    ...
                    <td>
                        <div className="dropdown">
                            ...
                            <div className="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                ...
                                <Link to={`/editservice/${item.id}`} className="dropdown-item"><i className="dw dw-edit2" /> Edit</Link>
                                <Link className="dropdown-item" onClick={() => swal({
                                    title: "Are you sure?",
                                    text: "You wanna delete this service",
                                    icon: "warning",
                                    buttons: true,
                                    dangerMode: true,
                                })
