...
function ViewUserReservation() {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/getUserByReservation`).then(res => {
            if (res.data.status === 200) {
                setUser(res.data.reservations);
                setLoading(false);
    ...
    const filteredUsers = ViewUser.filter((user) =>
        user.client.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
                    ...
                    <div class="contact-directory-box">
                        <div class="contact-dire-info text-center">
                            <div class="contact-avatar">
                                <img src={`http://localhost:8000/${item.client.image}`} width={250} height={250} />
                            </div>
                            <br></br>
                            <div class="contact-name">
                                <h5>{item.client.name}</h5>
                                <p>Email : {item.client.email}</p>
                                <p>Cin : {item.client.cin}</p>
                        ...
                        {/* <div class="view-contact">
                            ...
                            <Link className="btn btn-danger">Reset Settings</Link>
                        </div> */}
                    </div>
                </li>
            )
                ...
                <div className="page-header">
                    ...
                    <form>
                        <div className="form-group mb-0">
                            <input
                                type="text" className="form-control search-input" placeholder="Search Here" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
                        </div>
                    </form>
                </div>
