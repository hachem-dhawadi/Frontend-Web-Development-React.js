    ...
    if (loading) {
        ...
        dispaly_Servicedata = ViewService.map((item) => {
            ...
            return (
                <tr key={item.id}>
                    <td>{item.id}</td>
                    <td className="table-plus"><img src={`http://localhost:8000/${item.image}`} width={100} height={0} alt={item.name} /></td>
                    <td>{item.name}</td>
                    <td>{item.service.name}</td>
                    <td>{item.service.category.name}</td>
                    <td>{item.service.user.name}</td>
                    <td>{item.service.user.email}</td>
                    <td>{item.service.user.phone}</td>
                    <td>
                        <Link to={`/FindJob/${item.service.user.id}/${item.id}`} className="dropdown-item">Send <i className="dw dw-edit2" /></Link>
                    </td>
                </tr>
            )
        });
            ...
            <div className="main-container">
                <div className="pd-ltr-20">
                    ...
                    <div className="card-box mb-30">
                        <h2 className="h4 pd-20">List of jobs</h2>
