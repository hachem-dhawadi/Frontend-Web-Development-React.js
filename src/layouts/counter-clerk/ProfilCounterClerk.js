...
import LeftSidebar from "../counter-clerk/LeftSidebar";
import Navbar from "../superadmin/Navbar";

function ProfilCounterClerk(props) {
    ...
    useEffect(() => {
        ...
        axios.get('/api/alljobsbyauth')
            ...
            axios.get('/api/alljobsbyusersender')
            .then(response => {
                //setJobs(response.data.jobs);
                console.log("reciver");
                console.log(response.data.jobs);
    ...
    if (loading) {
        ...
        dispaly_Categorydata = job.map((item) => {
            ...
            return (
                <ul>
                    <li>
                    <div className="date">{item.room.name}</div>
                        <div className="task-name"><i className="ion-android-alarm-clock" />{item.tracking_no}</div>
                        <p>{item.created_at}</p>
                        <div className="task-time">Email : {item.reciver.email}</div>
                        <div className="task-time">Phone : {item.reciver.phone}</div>
                            ...
                            <div className="col-xl-4 col-lg-4 col-md-4 col-sm-12 mb-30">
                                <div className="pd-20 card-box height-100-p">
                                    <div className="profile-photo">
                                        <a href="modal" data-toggle="modal" data-target="#modal" className="edit-avatar"><i className="fa fa-eye" /></a>
...
export default ProfilCounterClerk;